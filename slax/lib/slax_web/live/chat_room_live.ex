defmodule SlaxWeb.ChatRoomLive do
    ...
    <div class="flex flex-col shrink-0 w-64 bg-slate-100">
      ...
      <div class="mt-4 overflow-auto">
        ...
        <div id="rooms-list">
          ...
          <div class="relative">
            <button
              class="flx items-center peer h-8 text-sm pl-8 pr-3 hover:bg-slate-300 cursor-pointer w-full"
            ...
            >
              <div class="w-full text-left">
                ...
                <.link
                  class="block select-none cursor-pointer whitespace-nowrap text-gray-800 hover:text-white px-6 py-1 block hover:bg-sky-600"
                  phx-click={JS.patch(~p"/rooms/#{@room}/new") |> show_modal("new-room-modal")}
                >
                  Create a new room
                </.link>
              </div>
    ...
    <div class="flex flex-col grow shadow-lg">
      <div class="flex justify-between items-center shrink-0 h-16 bg-white border-b border-slate-300 px-4">
        <div class="flex flex-col gap-1.5">
          ...
          <div
            class={["text-cs leading-none h-3.5", @hide_topic? && "text-slate-600"]}
  ...
  defp insert_date_dividers(messages, timezone) do
    ...
    |> Enum.flat_map(fn {date, message} -> [date | message] end)
  end
  ...
  defp maybe_insert_unread_marker(messages, last_read_at) do
    {read, unread} =
      Enum.split_while(messages, fn
        %Message{} = message -> Date.compare(message.inserted_at, last_read_at) != :gt
        _ -> true
      end)
  ...
  def handle_event("join-room", _, socket) do
    ...
    socket =
      assign(socket,
        joined?: true,
        rooms: Chat.list_joined_rooms_with_unread_count(current_user)
      )
  ...
  defp maybe_update_profile(socket, user) do
    if socket[:profile] && socket.assigns.current_user.id == user.id do
